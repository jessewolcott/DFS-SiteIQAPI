...
# Flatten tickets and export to CSV — no module required
[CmdletBinding()]
param(
    [PSCredential] $Credential,
    [string]       $BaseUri  = 'https://dfs.site-iq.com',
    [string]       $OutPath  = (Join-Path $PSScriptRoot 'SiteIQ-Tickets.csv')
)
...
# alerts is nested — flatten to ticket-level fields only
$Rows = $Tickets | ForEach-Object {
    [PSCustomObject]@{
        TicketID       = $_.ticketID
        ...
        WarrantyStatus = $_.warrantyStatus
        WarrantyDate   = $_.warrantyDate
        AlertCount     = @($_.alerts).Count
    }
}

$Rows | Export-Csv -Path $OutPath -NoTypeInformation
Write-Verbose "Wrote $(@($Rows).Count) rows to $OutPath"
$Rows
